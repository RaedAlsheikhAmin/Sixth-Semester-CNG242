...
-- Helper function: the main function to construct the tree
...
-- Base case: If the list of tuples is empty or the list of integers is empty, return an empty list.
helper [] _ _ = []
helper _ [] _ = []
-- Recursive cases:
helper ((name, emp):restnames) (numchildren:restchildren) childforpreviousnode
-- If the number of children in the previous node is greater than 0 and the current number of children is 0,
-- create a UniTree node with the given name and employee number and append it to the result.
-- Then recursively call 'helper' with the remaining tuples and integers, and decrement the number of children in the previous node
    ...
    -- If the number of children in the previous node is greater than 0 and the current number of children is 1,
    -- create a UniTree node with the given name and employee number, and recursively call 'helper' with the remaining tuples and integers.
    -- Also, advance to the next tuple and integer.
    ...
    -- If the number of children in the previous node is greater than 0 and the current number of children is not 0 or 1,
    -- create a UniTree node with the given name and employee number, and recursively call 'helper' with the remaining tuples and integers.
    | childforpreviousnode > 0 && numchildren /= 0 =
        [Node (name, emp) numchildren (helper restnames restchildren numchildren)]
    -- If none of the above conditions are met, return a UniTree node with the given name and employee number, and an empty list of children.
    | otherwise =
